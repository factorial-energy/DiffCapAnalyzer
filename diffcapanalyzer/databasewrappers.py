...
def process_data(
    ...
    print("database got table")
    core_file_name = get_filename_pref(file_name)
    print("database got file name")
    if core_file_name + "CleanSet" in names_list:
        print("database returned")
        return
    else:
        parse_update_master(core_file_name, database_name, datatype, decoded_dataframe)
        print("database parse update master")
...
def parse_update_master(core_file_name, database_name, datatype, decoded_dataframe):
    ...
    print("database update master table")
    return
...
def get_db_filenames(database_name):
    ...
    c = con.cursor()
    names_list = []
    for row in c.execute("""SELECT Dataset_Name FROM master_table"""):
