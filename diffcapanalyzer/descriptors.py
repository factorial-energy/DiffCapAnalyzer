...
def generate_model(df_clean, filename, peak_thresh, database):
    ...
    param_df = pd.DataFrame(
        columns=["Cycle", "Model_Parameters_charge", "Model_Parameters_discharge"]
    )
    ...
    for cyc in df_clean[cycle_ind_col].unique():
        try:
            new_df_mody, model_c_vals, model_d_vals, peak_heights_c, peak_heights_d = (
                get_model_dfs(df_clean, datatype, cyc, lenmax, peak_thresh)
            )
    ...
    update_database_newtable(
        mod_pointsdf, filename.split(".")[0] + "-ModPoints", database
    )
    ...
    if len(cycles_no_models) > 0:
        return (
            "That model has been added to the database."
            + "No model was generated for Cycle(s) "
            + str(cycles_no_models)
        )
...
def get_model_dfs(df_clean, datatype, cyc, lenmax, peak_thresh):
    ...
    i_charge, volts_i_ch, peak_heights_c = peak_finder(
        clean_charge, "c", windowlength, polyorder, datatype, lenmax, peak_thresh
    )
    ...
    if model_d is not None:
        ...
        new_df_mody_d = pd.concat(
            [-myseries_d, V_series_d, dQdV_series_d, clean_discharge[cycle_ind_col]],
            axis=1,
        )
...
def my_pseudovoigt(x, cent, amp, fract, sigma):
    ...
    """
    sig_g = sigma / np.sqrt(2 * np.log(2))
    # calculate the sigma_g parameter for the gaussian distribution
    part1 = (((1 - fract) * amp) / (sig_g * np.sqrt(2 * np.pi))) * np.exp(
        (-((x - cent) ** 2)) / (2 * sig_g**2)
    )
...
def param_dicts_to_df(mod_params_name, database):
    ...
    for i in range(len(mod_params_df)):
        ...
        for item in charge_keys:
            ...
            new_dict_charge.update(
                {
                    "c_area_peak_" + str(peaknum): PeakArea,
        ...
        for item in discharge_keys:
            ...
            new_dict_discharge.update(
                {
                    "d_area_peak_" + str(peaknum): PeakArea,
...
def model_gen(V_series, dQdV_series, cd, i, cyc, thresh):
    ...
    if all(newi) is False or len(newi) < 1:
        ...
        for index in newi:
            ...
            par[fraction].set(0.5, min=0, max=1)
...
def dfsortpeakvals(mydf, cd):
    ...
    filter_col_actheight = [
        col for col in mydf if str(col).startswith(cd + "_rawheight")
    ]
        ...
        for key in groupdict:
            ...
            for j in range(len(filter_col_loc)):
                ...
                for i in range(len(mydf)):
                    ...
                    if mydf.loc[i, (filter_col_loc[j])] >= min(
                        list(groupdict[key].allpeaks)
                    ) and mydf.loc[i, (filter_col_loc[j])] <= max(
                        list(groupdict[key].allpeaks)
                    ):
                        mydf.loc[i, ("sortedloc-" + cd + "-" + str(count))] = mydf.loc[
                            i, (filter_col_loc[j])
                        ]
                        mydf.loc[i, ("sortedheight-" + cd + "-" + str(count))] = (
                            mydf.loc[i, (filter_col_height[j])]
                        )
                        mydf.loc[i, ("sortedarea-" + cd + "-" + str(count))] = mydf.loc[
                            i, (filter_col_area[j])
                        ]
                        mydf.loc[i, ("sortedSIGMA-" + cd + "-" + str(count))] = (
                            mydf.loc[i, (filter_col_sigma[j])]
                        )
                        mydf.loc[i, ("sortedamplitude-" + cd + "-" + str(count))] = (
                            mydf.loc[i, (filter_col_ampl[j])]
                        )
                        mydf.loc[i, ("sortedfwhm-" + cd + "-" + str(count))] = mydf.loc[
                            i, (filter_col_fwhm[j])
                        ]
