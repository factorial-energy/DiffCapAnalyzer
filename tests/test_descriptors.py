...
test_import_dictionary = {
    "test_battery-CleanCycle1": test_cycle1_df,
...
def test_get_model_dfs():
    ...
    new_df_mody, model_c_vals, model_d_vals, peak_heights_c, peak_heights_d = (
        get_model_dfs(df_clean, test_datatype, cyc, lenmax, peak_thresh)
    )
...
def test_get_model_dfs_for_maccor():
    ...
    peak_thresh = 0.7
    new_df_mody, model_c_vals, model_d_vals, peak_heights_c, peak_heights_d = (
        get_model_dfs(df_clean, test_datatype_mac, cyc, lenmax, peak_thresh)
    )
...
def test_generate_model():
    ...
    feedback = generate_model(df_clean, filename_pref, peak_thresh, test_db)
...
def test_cd_dataframe():
    ...
    c_result = cd_dataframe(
        test_cycle1_df["Voltage(V)"], test_cycle1_df["Smoothed_dQ/dV"], "c"
    )
    c_sigx = c_result[0]
    c_sigy = c_result[1]
    ...
    d_result = cd_dataframe(
        test_cycle1_df["Voltage(V)"], test_cycle1_df["Smoothed_dQ/dV"], "d"
    )
    d_sigx = d_result[0]
    d_sigy = d_result[1]
...
def test_model_gen():
    ...
    par_result, mod_result, i_result = model_gen(
        test_cycle1_df["Voltage(V)"], test_cycle1_df["Smoothed_dQ/dV"], "c", [2], 1, 0.4
    )
