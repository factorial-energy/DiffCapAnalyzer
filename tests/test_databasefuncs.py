...
def test_get_file_from_database():
    ...
    result = get_file_from_database(
        "my_other_amazing_file", "another_amazing_database.db"
    )
    assert pd.DataFrame.equals(result, df)
    neg_result = get_file_from_database("something_else", "another_amazing_database.db")
