...
app = dash.Dash(
    ...
    external_stylesheets=[
        ...
        {"href": "https://codepen.io/chriddyp/pen/bWLwgP.css", "rel": "stylesheet"},
    ],
)
...
app.layout = html.Div(
    [
        html.Div([html.H1("Differential Capacity Analyzer")]),
        html.Div(
            [
                html.Div(
                    [
                    ...
                    [
                        ...
                        html.Div(
                            [
                                ...
                                html.Div(
                                    [
                                        "Once data is uploaded, select the new data from the dropdown menu to the left (refresh the page if necessary)."
                                    ],
                                    style={
                                        "margin-left": "50px",
                                        "font-style": "italic",
        ...
        html.Div(
            [
                ...
                html.Div(
                    [html.Br()], style={"width": "89%", "display": "inline-block"}
                ),
                html.Br(),
                html.Div(
                    [
                        dcc.RadioItems(
                            id="show-model-fig1",
                            options=[
                                {"label": "Show Model", "value": "showmodel"},
            ...
            [
                        ...
                        html.Div(
                            [
                                html.Div(
                                    ...
                                    style={"width": "75%", "font-style": "italic"},
                                ),
                                dcc.RadioItems(
                                    id="cd-to-plot",
                                    options=[
                ...
                html.Div(
                    [
                        ...
                        html.Div(
                            [
                                "After updating the threshold, you can update the preview of the model"
                                + " and then update the database once the model appears to be optimal."
                            ],
                            style={"font-style": "italic"},
                        ),
                        html.Div(id="update-model-ans"),
                        html.Button(
                            "Update Preview of Model", id="update-model-button"
                        ),
                        html.Button(
                            "Update Model in Database", id="update-model-indb-button"
                        ),
                        html.Div(
                            [
                                dcc.Checklist(
    ...
    try:
        ...
        if contents is None:
            ...
            return html.Div(
                [
                    parse_contents(
                        decoded, filename, value, database, windowlength, polyorder
                    )
                ]
            )
...
@app.callback(
    Output("update-model-ans", "children"),
    [
...
def update_model_indb(filename, n_clicks, new_peak_thresh):
    if n_clicks is not None:
        ...
        feedback_str = generate_model(df_clean, filename, new_peak_thresh, database)
...
@app.callback(  # update slider
    Output("cycle--slider", "max"), [Input("available-data", "value")]
)
...
def update_figure2(
    ...
    fig = plotly.subplots.make_subplots(
        ...
        subplot_titles=(
            "Descriptors",
            "Example Data for Model Tuning (Cycle " + str(int(selected_step)) + ")",
        ),
        shared_xaxes=True,
    )
    marker = {"color": ["#0074D9"]}
    if peak_vals_df is not None:
        if n_clicks is not None:
            ...
            new_df_mody, model_c_vals, model_d_vals, peak_heights_c, peak_heights_d = (
                get_model_dfs(dff_data, datatype, selected_step, lenmax, peak_thresh)
            )
            dff_mod = new_df_mody
                ...
                fig.append_trace(
                    {
                        "x": dff_mod[volt_col],
                        "y": (
                            (c_amplitude / (c_sigma * ((2 * 3.14159) ** 0.5)))
                            * np.exp(
                ...
                fig.append_trace(
                    {
                        "x": dff_mod[volt_col],
                        "y": -(
                            (d_amplitude / (d_sigma * ((2 * 3.14159) ** 0.5)))
                            * np.exp(
    ...
    fig["layout"]["yaxis2"].update(
        title="dQ/dV",
        range=[dff_data["Smoothed_dQ/dV"].min(), dff_data["Smoothed_dQ/dV"].max()],
    )
    fig["layout"]["height"] = 600
    fig["layout"]["margin"] = {"l": 40, "r": 10, "t": 60, "b": 200}
    return fig
...
@app.callback(Output("my-link-1", "href"), [Input("available-data", "value")])
...
@app.callback(Output("my-link-2", "href"), [Input("available-data", "value")])
...
@app.callback(Output("my-link-3", "href"), [Input("available-data", "value")])
